...
from . import lri_forward, opt, seidel, util
...
def calibrate(
    ...
    if seidel_coeffs is not None:
        ...
        psf_locations, center, calib_image = util.get_calib_info(
            calib_image, center, [desired_dim, desired_dim], centered_psf=centered_psf
        )  # going to be in (x,y) relative to center. Center will be row, col from top left of raw_image

        """Need to generalize this to multishot"""
        if verbose:
            print("estimating seidel coefficients...")
        seidel_coeffs = opt.estimate_coeffs_ss(
            calib_image, psf_locations, sys_params, opt_params, device
        )
    ...
    psf_stack_roft = seidel.compute_psfs(
        seidel_coeffs.cpu(),
        point_list,
        sys_params,
        polar=True,
...
def blur(
    ...
    psf_stack_roft,
    verbose=False,
    artifact_correction=0.5,
    device=torch.device("cpu"),
):
    # diff image stuff
    if artifact_correction:
        constant = torch.ones(obj.shape, device=device) * artifact_correction
        constant_response = lri_forward.forward(
            constant, psf_stack_roft, method="normal", device=device, verbose=verbose
        )
    ...
    return lri_forward.forward(
        obj, psf_stack_roft, method="normal", device=device, verbose=verbose, diff=diff
    )
