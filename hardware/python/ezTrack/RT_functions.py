...
class Video:
    ...
    def display(self, show_xy=True, show_dif=True):
        ...
        while True:
            ...
            if show_xy == True and self.track_yx is not None:
                markposition = (int(self.track_yx[1]), int(self.track_yx[0]))
    ...
    def get_frames(self):
        ...
        while self.started:
            ...
            if ret == True:
                ...
                if self.scale != 1:
                    frame = cv2.resize(
                        frame, (self.scale_w, self.scale_h), cv2.INTER_NEAREST
                    )
    ...
    def locate(self, frame):
        ...
        if self.track_rmvwire == True:
            ...
            if krn_violation:
                print(
                    "WARNING: wire_krn too large. Reverting to rmv_wire=False for current frame"
                )
    ...
    def crop_define(self):
        ...
        image = hv_baseimage(frame=self.frame, text="Crop as Desired")
    ...
    def mask_define(self):
        ...
        image = hv_baseimage(frame=self.frame, text="Draw Regions to be Excluded")

        # Create polygon element on which to draw and connect via stream to PolyDraw drawing tool
        self.mask = dict(mask=None)
        poly = hv.Polygons([])
        self.mask["stream"] = streams.PolyDraw(
            source=poly, drag=True, show_vertices=True
        )
    ...
    def roi_define(self, names=None):
        ...
        image = hv_baseimage(
            frame=self.frame,
            text="No Regions to Draw"
            if nobjects == 0
            else "Draw Regions: " + ", ".join(self.roi_names),
        )

        # Create polygon element on which to draw and connect via stream to PolyDraw drawing tool
        poly = hv.Polygons([])
        self.roi_stream = streams.PolyDraw(
            source=poly, drag=True, num_objects=nobjects, show_vertices=True
        )
        poly.opts(fill_alpha=0.3, active_tools=["poly_draw"])
    ...
    def writer_writer(
        ...
        if save_vid:
            writer = cv2.VideoWriter(
                filename=vpath,
                fourcc=cv2.VideoWriter_fourcc(*compression)
                if compression in ["MJPG", "FFV1", "XVID"]
                else 0,
                fps=fps,
        ...
        while not stopsig.is_set() or emptyq.is_set():
            try:
                ...
                if save_csv:
                    ...
                    pd.DataFrame(
                        {k: v for k, v in dcsv.items() if k in dkeys}, index=[0]
